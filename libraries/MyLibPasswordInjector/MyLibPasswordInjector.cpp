...
void readJoystick(int debaounceDelay) {
  ...
  if (analogReadChannel(XAXISPIN) > 1000) {
    
    arrowPointing++;
    updateView(arrowPointing);
    delay(debaounceDelay);
  } else if (analogReadChannel(XAXISPIN) < 20) {
  ...
  if (analogReadChannel(YAXISPIN) > 1000) {
    
    if(arrowPointing == 3){arrowPointing = 1;}
    else{arrowPointing++;}
    updateView(arrowPointing);
    delay(debaounceDelay);
  } else if (analogReadChannel(YAXISPIN) < 20) {
  ...
  if (analogReadChannel(BUTTONPIN) < 10) {
    if(arrowPointing == 1){
      lcd.clear();
      lcd.setCursor(0, 0);
